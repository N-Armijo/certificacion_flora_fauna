...
const especiesFiltradas = computed(() => {
  ...
  if (filtroTipo.value) {
    resultado = resultado.filter(
      (especie) => especie.tipo.toLowerCase() === filtroTipo.value.toLowerCase(),
    )
  }
  ...
  if (filtroRegion.value) {
    resultado = resultado.filter((especie) =>
      especie.ubicacion.toLowerCase().includes(filtroRegion.value.toLowerCase()),
    )
  }
  ...
  if (filtroEstado.value) {
    resultado = resultado.filter(
      (especie) => especie.estadoConservacion.toLowerCase() === filtroEstado.value.toLowerCase(),
    )
  }
    ...
    <div class="row mb-3">
      ...
      <div class="col-md-4">
        <select v-model="filtroRegion" class="form-select">
          ...
          <option value="Cordillera de los Andes, Patagonia">
            Cordillera de los Andes, Patagonia
          </option>
          <option value="patagonia">Patagonia</option>
          <option value="Zona Central y Sur de Chile">Zona Central y Sur de Chile</option>
          <option value="Isla de Chiloé y Región de Los Lagos">
            Isla de Chiloé y Región de Los Lagos
          </option>
          <option value="Zona Austral de Chile">Zona Austral de Chile</option>
          <option value="Cordillera de los Andes y Cordillera de la Costa">
            Cordillera de los Andes y Cordillera de la Costa
          </option>
          <option value="Cordillera de los Andes, Patagonia y Región del Maule">
            Cordillera de los Andes, Patagonia y Región del Maule
          </option>
